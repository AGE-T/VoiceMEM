...
| --- | --- | --- |
| `VoiceMemAgent_v0.10.4.zip` | **The product.** Full agent source, machine-independent (no runtime state inside: no `.venv`, models, or memory). Install = unzip + `START.bat`. | 2.7 MB |
...
1. Unzip `VoiceMemAgent_v0.10.4.zip` into any folder (e.g. `F:\Voicemem\VoiceMemAgent`).
...
## The verification story (v0.10.4)
...
- **Test gate (v0.10.4 release gate):** 1436 passed / 0 regressions /
  29 skipped (7 pinned sandbox env-gap — documented in the pinned
  baseline, green on the target machine); deep validation PASS
  (embedding, memory, memory-semantics, release, runtime-deps) on
  fingerprint
  70ba38d83904cc6c2c3be765bca28432931a457c2fc4a07aa670abec722b51ff
...
stabil verzió a **v0.10.4**. Mit találhat itt:

- **`VoiceMemAgent_v0.10.4.zip`** — maga a termék. Gépfüggetlen forrás,
