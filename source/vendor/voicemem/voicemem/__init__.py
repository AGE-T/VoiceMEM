...
# ── [CONTROLLED FORK - patch VM-LOCAL-006] 受控分叉身份 ──────────────────────────
# 本包是 VoiceMemAgent 仓库里的受控 VoiceMem 源（vendor/voicemem，随仓库发布），
# 不是从上游 git 克隆的安装。上游身份由仓库根的 VOICEMEM_PIN.json 权威记录；
# 这组常量让运行期（import voicemem 之后）能直接验证「装的就是受控源、
# pin 的就是那个上游 commit」。UPSTREAM_POLICY.md 规定上游变更的采纳流程。
# 注意：__version__（上游 pip 元数据 0.2.3）**不是**身份标识——commit 才是。
CONTROLLED_FORK = True
CONTROLLED_UPSTREAM_REPO = "https://github.com/xzf-thu/VoiceMem.git"
CONTROLLED_UPSTREAM_COMMIT = "e8384e087bd2f44eb05fc7ae1a3c525ea8244179"
CONTROLLED_UPSTREAM_TAG = "v0.0.1"
CONTROLLED_PATCHES = (
    "VM-LOCAL-001",  # orchestrator._embed_text -> injected/local E5 (no OpenAI embeddings)
    "VM-LOCAL-002",  # leftbrain repo default embedder -> LocalE5Embedder
    "VM-LOCAL-003",  # default_utils embedding factory -> LocalE5Embedder
    "VM-LOCAL-004",  # traits_store._vec: loud failure (no silent NULL embeddings)
    "VM-LOCAL-005",  # delete_memory opt-in safety guard
    "VM-LOCAL-006",  # controlled-fork runtime identity (this block)
    ...
    "UPSTREAM-e3cc965",  # memory event dates (observed_at) validated before use
)
