...
def default_utils(base_url, memory_root):
    def embedding():
        # [CONTROLLED FORK - patch VM-LOCAL-003]
        # Upstream default was OpenAILocalEmbedder (remote OpenAI Embeddings
        # API). Controlled-fork invariant (VOICEMEM_PIN.json): memory
        # embeddings are the LOCAL multilingual E5 — no remote embedding
        # service may ever be the silent default. Callers that explicitly
        # inject their own TextEmbedder (including OpenAILocalEmbedder)
        # still control that choice; this only changes the DEFAULT.
