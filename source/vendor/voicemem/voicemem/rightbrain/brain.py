...
#:
#: [ports upstream f535f9d] 换 embedder 之后这个数必须重置——不同模型的相似度分布完全不同，
#: 写死一个常量只对其中一个有效。实测（同一批 130 条判断）：
...
def trait_min_sim(dim: int | None) -> float:
    """[ports upstream f535f9d] 这个 embedder 该用多高的门槛。显式设了环境变量就一切照它。"""
...
文本字段各 ≤{n} 字，语言跟用户那句一致。"""
    ...
    def _execute_heartnote_deletes(self, rb_repo, full_ids: list[str]) -> int:
        """[CONTROLLED FORK - patch VM-LOCAL-007] Right-brain DELETE safety guard.

        Every hard-delete of right-brain (emotional) memory in run_cleanup —
        the LLM's ``delete_ids`` decision on the Ingest hot path — must pass
        the same opt-in gate as the left brain (VM-LOCAL-005). Default is
        DENY: without ``VOICEMEM_ALLOW_MEMORY_DELETE=1`` no row is removed;
        the request is logged loudly and 0 is returned. The supersede branch
        of run_cleanup is always non-destructive and never passes through
        here. This closes audit finding CD-1 (P0): an uncontrolled LLM
        decision could previously destroy emotional memory irreversibly.

        Returns the number of heartnotes actually deleted (0 when blocked).
        """
        import logging
        if os.environ.get("VOICEMEM_ALLOW_MEMORY_DELETE", "") != "1":
            logging.getLogger(__name__).warning(
                "run_cleanup: LLM requested hard-delete of %d right-brain "
                "heartnote(s) (ids: %s) — BLOCKED by the controlled-fork "
                "safety guard (default deny). Set VOICEMEM_ALLOW_MEMORY_DELETE=1 "
                "to allow destructive emotional-memory deletion; the supersede "
                "branch of run_cleanup is non-destructive and unaffected.",
                len(full_ids), ", ".join(full_ids[:8]))
            print(f"[Cleanup] DELETE BLOCKED (VM-LOCAL-007): {len(full_ids)} 条右脑记忆保留"
                  f"（VOICEMEM_ALLOW_MEMORY_DELETE 未开启）")
            return 0
        import sqlite3
    ...
    def run_cleanup(self) -> None:
        ...
        try:
            ...
            if full_ids:
                n_deleted = self._execute_heartnote_deletes(rb_repo, full_ids)
                if n_deleted:
                    print(f"[Cleanup] 清洁完成，删除 {n_deleted} 条右脑记忆")
            else:
                print("[Cleanup] 无需删除")
            ...
            for p in pairs:
                ...
                if keep_old:
                    ...
                    self._execute_heartnote_deletes(rb_repo, [old_full])
                marked += 1
            if marked:
