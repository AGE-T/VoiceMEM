...
class LeftBrainMemoryRepositoryV2(LeftBrainMemoryRepository):
    ...
    **kwargs:
    ...
    def update_memory(self, memory_id: str, new_text: str, **kwargs) -> str | bool | None:  # type: ignore[override]
        ...
        [CONTROLLED FORK - patch VM-LOCAL-008] 父类现在返回**新行 id**
        （非破坏性 UPDATE：追加 + supersede）。slot 标签打在**新行**上
        （当前值）；旧行的标签原样保留（历史）。"""
        user_id = kwargs.get("user_id")
        new_id = super().update_memory(memory_id, new_text, **kwargs)
        if new_id and user_id and self._cognitive_store is not None:
            self._tag_memory_slots_v2([(str(new_id), new_text.strip())], user_id=user_id)
        return new_id
