...
#: [ports upstream 91d2e42] 维度不符只提醒一次，别每轮刷屏。
_WARNED_DIM: set = set()
        ...
        *,
        ...
        if mismatched and not _WARNED_DIM:
            ...
            logging.getLogger(__name__).warning(
                "graph_entities: %d entity vectors have a dimension that does "
                "not match the current embedder — skipped. After an embedder "
                "change old vectors are stale; semantic entity dedup is "
                "degraded until they are re-embedded.", mismatched)
