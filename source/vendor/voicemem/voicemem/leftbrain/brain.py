...
def _dedupe_near(hits: list) -> list:
    ...
    [CONTROLLED FORK - patch VM-LOCAL-008] 显式取代对**不去重**："偏好是A"
    和"偏好改成B"是两条独立观察（值不同），不是同一事实的两版措辞——
    历史值必须能在检索结果里活着。近重复折叠只针对"同值不同措辞"。
    """
    kept, grams = [], []
    for h in hits:
        g = _trigrams(getattr(h, "text", ""))
        h_id = getattr(h, "memory_id", "")
        h_sup = getattr(h, "superseded_by", "") or ""
        dup = False
        for k, g0 in zip(kept, grams):
            # 显式取代对（A 被 B 取代 / B 取代 A）永远双双保留。
            if h_sup and h_sup == getattr(k, "memory_id", ""):
                continue
            if h_id and h_id == getattr(k, "superseded_by", ""):
                continue
            inter = len(g & g0)
    ...
    def _get_repo(self):
        with self._lock:
            if "repo" not in self._cache:
                ...
                # [CONTROLLED FORK - patch VM-LOCAL-002]
                # Upstream default was OpenAILocalEmbedder (remote OpenAI
                # Embeddings API via base_url). Our llama-server endpoint has
                # no /v1/embeddings route, so that default could only fail.
                # Controlled-fork invariant (VOICEMEM_PIN.json): the memory
                # embedder is the LOCAL multilingual E5 unless an embedder is
                # explicitly injected — never a remote embedding service.
                # OpenAILocalEmbedder remains available for callers that
                # explicitly inject it; it is no longer the silent default.
