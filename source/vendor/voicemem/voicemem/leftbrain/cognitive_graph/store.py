...
class CognitiveGraphStore:
    ...
    def delete_memory_record(self, memory_id: str) -> int:
        """[CONTROLLED FORK - patch VM-LOCAL-009] 单条记忆的图级联删除。

        一个被允许的左脑删除（向量层 VM-LOCAL-005 默认拒绝，opt-in 才删）
        绝不能在图里留下看起来仍然权威的陈旧行（审计发现 F-H）。这里删掉
        同一个 space SQLite 里所有引用该 memory_id 的行：

        · ``memories``            — 认知图包装行（实体图检索的入口）
        · ``entity_memory_links`` — 实体 ↔ 记忆链接
        · ``memory_tags``         — V2 多标签 slot 标签（V1 老库没有这张表）
        · ``graph_entity_memories`` — slot_split 图层链接（老库可能没有）

        返回删除的 memories 包装行数（0 = 本来就没有图记录）。
        """
        import sqlite3
        with self._conn() as c:
            # The store enforces foreign keys (PRAGMA foreign_keys=ON): the
            # referencing rows (links/tags) must go BEFORE the wrapper row.
            c.execute("DELETE FROM entity_memory_links WHERE memory_id=?", (memory_id,))
            for ddl_sql in (
                "DELETE FROM memory_tags WHERE memory_id=?",
                "DELETE FROM graph_entity_memories WHERE memory_id=?",
            ):
                try:
                    c.execute(ddl_sql, (memory_id,))
                except sqlite3.OperationalError:
                    pass  # 该图层在这块老库上不存在（V1 schema）
