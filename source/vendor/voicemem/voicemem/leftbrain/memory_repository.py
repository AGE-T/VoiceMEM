...
class LeftBrainMemoryRepository:
    ...
    def update_memory(self, memory_id: str, new_text: str,
        ...
        旧行只加 ``superseded_by`` 元数据、原文保留。本层同步做两件事：

        · JSON 镜像：**不再覆盖**旧条目 —— 旧条目标注 ``superseded_by``，
          追加新条目（带 ``supersedes`` + 会话/事件时间）；
        ...
        if new_id:
            ...
            store["results"].append(new_entry)
            ...
            if user_id is not None and self._cognitive_store is not None and self._cognitive_annotator is not None:
                try:
                    ...
                    if annotated:
                        # 新事实挂新 id：当前值的图记录；旧行图记录不动（历史）。
                        self._cognitive_store.ingest_annotated_fact(
                            user_id, annotated[0], [new_id])
    ...
    def delete_memory(self, memory_id: str) -> bool:
        ...
        [CONTROLLED FORK - patch VM-LOCAL-009] 向量层的删除仍是 opt-in
        （VM-LOCAL-005，默认拒绝）。一旦真删了，认知图里所有引用这条
        memory_id 的行（memories 包装行、entity_memory_links、
        memory_tags、graph_entity_memories）必须一起删掉——否则图里留着
        "看起来仍然权威"的陈旧真值（审计发现 F-H：孤儿图记录）。"""
        ...
        if deleted:
            ...
            if self._cognitive_store is not None:
                try:
                    self._cognitive_store.delete_memory_record(memory_id)
                except Exception as _cog_err:
                    import logging
                    logging.getLogger(__name__).warning(
                        "DELETE 认知图级联失败（可能残留孤儿行, id=%s）: %s",
                        memory_id, _cog_err)
        return deleted
