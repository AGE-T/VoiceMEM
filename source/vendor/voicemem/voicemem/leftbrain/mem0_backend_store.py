...
class Mem0BackendStore:
    ...
    def update_memory(self, memory_id: str, new_text: str,
        ...
        place: ``mem0.update(memory_id, text=new_text)``. A previous fact
        then became unrecoverable purely because an LLM ConflictResolver
        emitted UPDATE — audit finding CD-2 (P0): the memory engine behaved
        as a latest-value store, not a memory.

        New semantics (append + explicit supersession):

        1. the new observation becomes a NEW row (new mem0 id) carrying
           ``supersedes: <old_id>`` plus the usual provenance metadata
           (session_id, event-time ``created_at``);
        2. the old row is NEVER text-rewritten — only its metadata gains
           ``superseded_by: <new_id>`` and ``superseded_at`` (mem0's
           metadata-merge update keeps text/created_at when text is None);
        ...
        update is REFUSED (loud log) — we never fall back to the destructive
        overwrite.

        Returns the new memory id on success (truthy), ``False`` on refusal
        or failure (callers historically treated the return as a bool; a
        non-empty id string keeps that contract).
        """
        import logging
        t = new_text.strip()
        if not t:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "update_memory(%s): cannot read existing row (%s) — refusing "
                "to update blind", memory_id, e)
            return False
        if not existing:
            logging.getLogger(__name__).warning(
                "update_memory(%s): memory not found — nothing to supersede", memory_id)
            return False
        owner = (user_id or str(existing.get("user_id") or "")).strip()
        if not owner:
            logging.getLogger(__name__).warning(
                "update_memory(%s): no user_id to own the new observation — "
                "REFUSING (VM-LOCAL-008 never falls back to in-place overwrite)",
                memory_id)
            return False
        ...
        try:
            ...
            if not entries:
                return False
            new_id = str(entries[0]["id"])
        except Exception as e:
            logging.getLogger(__name__).warning(
                "update_memory(%s): appending the new observation failed (%s)", memory_id, e)
            return False
        # Mark the old observation as superseded. metadata-only update: mem0
        # merges the dict into the payload and keeps the text (data=None →
        ...
        try:
            ...
            # pair will lack the explicit link, which the caller can repair.
            logging.getLogger(__name__).warning(
                "update_memory(%s): superseded_by marking failed (%s) — new "
                "row %s kept, link missing", memory_id, e, new_id)
            return new_id
        return new_id
