...
# [CONTROLLED FORK - patch VM-LOCAL-001]
#: embed_cache 的键：本受控分叉里 ``_embed_text`` 只走本地多语言 E5（或注入的
#: embedder），缓存键固定为这个本地标识——与任何 OpenAI 模型名都不相同，因此
#: 绝不可能命中（也绝不污染）旧 OpenAI 通道留下的缓存条目。
_LOCAL_E5_CACHE_KEY = "local-multilingual-e5-small:controlled"
...
class Orchestrator:
    ...
    def _embed_text(self, text: str) -> list[float]:
        ...
        [CONTROLLED FORK - patch VM-LOCAL-001, based on upstream 961efe8]
        ...
        **Local divergence from upstream 961efe8**（intentional, see
        VOICEMEM_PIN.json / UPSTREAM_POLICY.md in the controlling repo）:
        upstream keeps the OpenAI Embeddings API as the *default* when no
        embedder is injected. This controlled fork REMOVES that remote path
        entirely — the default is the local multilingual E5 as well. The
        deployment invariant is: any VoiceMem memory embedding goes through
        the explicitly injected local multilingual E5 (or, when nothing is
        injected, the same local E5 via LocalE5Embedder) — never an external
        OpenAI embedding service. Our llama-server endpoint has no
        /v1/embeddings route at all, so the old default could only ever
        fail (and be silently swallowed downstream).
        """
        ...
        # embed 过一轮，一模一样的字符串没必要再算一次。
        from voicemem.utils.common import embed_cache
        return embed_cache.resolve(_LOCAL_E5_CACHE_KEY, [text], self._embed_uncached)[0]

    def _embed_uncached(self, texts: list[str]) -> list[list[float]]:
        """[CONTROLLED FORK - patch VM-LOCAL-001] Local multilingual E5 only.

        The upstream implementation called the OpenAI Embeddings API
        (``client.embeddings.create`` against ``self._base_url``). That call
        path is REMOVED in this controlled fork: there is no embedding
        endpoint on our OpenAI-compatible llama-server, and a silent remote
        fallback violates the embedding policy (all memory embeddings are
        local, deterministic, offline). If the local model cannot be loaded
        the failure propagates — it is never masked by an OpenAI call.
        """
        from voicemem.leftbrain.local_e5_embedder import LocalE5Embedder
        return LocalE5Embedder().embed_texts(texts)
