...
| File | Role |
| --- | --- |
| `install_b11073.ps1` | installer: pinned download → SHA-256 gate → staged extraction → 24-file measured closure → per-file hash + build-identity verification → manifest |
...
| `start_llama_server_experimental.ps1` | launcher: starts the downloaded b11073 with the validated profile, waits for `/health`, verifies the version (binary banner + live system fingerprint), logs the effective configuration |
| `verify_b11073.ps1` | end-to-end verification (install integrity, production-untouched pins, live server, client transport; `-ThroughAppClient` runs the real app client) |
...
The launcher: checks the binary's `--version` banner is **b11073** (fails
closed otherwise), prepends the production `bin\` to the child PATH so
`cublas64_13.dll` resolves from the existing install, waits for `/health`,
then verifies the RUNNING server via its `system_fingerprint` (a real
1-token completion must report `b11073-…`), and writes the effective
configuration (resolved model, flags, PID, hashes) to
...
ctx 32768); `/health` on 8081 is OK; the running server identifies as
b11073; and the OpenAI-compatible chat-completions transport the application
uses answers.
...
| launcher: `a binaris VERZIOJA NEM b11073` | something replaced the installed binary — delete `experimental\llama_b11073\bin\` and re-run `install_b11073.ps1 -Force`. |
...
| app still on 8080 | you launched the normal `START.bat` (production session) instead of the generated `start_voicemem_experimental.ps1` — that is by design. |
...
* The installer's script logic was additionally executed in this sandbox
  ...
  Windows-specific parts (process start of the PE server, `.venv` client
  probe) are the operator's target-side steps documented above.
